                        ...
                        <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-[700px] sm:w-full">
                            ...
                            <header class="py-3 px-4 flex justify-between items-center">
                                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                    {{ skill.id ? `Update image: "${props.skill.name}"` : 'Create new image' }}
                                </DialogTitle>
                                <button
                                    @click="closeModal()"
                            ...
                            <form @submit.prevent="onSubmit">
                                <div class="bg-white px-4 pt-5 pb-4">
                                    <CustomInput class="mb-2" v-model="skill.name" label="Skill Name" />
                                    <CustomInput type="file" class="mb-2" label="Image" @change="file => skill.image = file" />
                                    <CustomInput type="checkbox" class="mb-2" v-model="skill.published" label="Published"/>
                                </div>
...
const skill = ref({
    ...
    published: props.skill.published,
})
console.log(skill)
...
function onSubmit(){
    loading.value = true
    if (skill.value.id){
        store.dispatch('updateSkill', skill.value)
        .then(response => {
            loading.value = false;
        ...
        store.dispatch('createSkill', skill.value)
        .then(response => {
            loading.value = false;
